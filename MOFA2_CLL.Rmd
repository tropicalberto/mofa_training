...
weights %>% 
  arrange(desc(value)) %>% filter(view != 'Mutations', factor == 'Factor1', view=='Methylation')
...
high_rna_genes <- weights %>% 
  arrange(desc(value)) %>% filter(view != 'Mutations', factor == 'Factor1', view=='mRNA') %>% 
  slice(1:20)

as_tibble(CLL_data$mRNA) %>% mutate(ensgene = rownames(CLL_data$mRNA)) %>%
  left_join(gene_metadata %>% dplyr::select(ens_id, symbol), by=c('ensgene'='ens_id')) %>%
  filter(symbol %in% high_rna_genes$feature) %>%
  gather(key='sample',value='gexp', -ensgene, -symbol) %>%
  left_join(CLL_metadata, by='sample') %>% 
  filter(!is.na(IGHV)) %>% mutate(IGHV=factor(IGHV)) %>%
  ggplot(aes(x=IGHV, y=gexp)) +
  geom_boxplot() + facet_wrap(~symbol, scales='free_y') + stat_compare_means(method='t.test') +
  scale_y_log10(expand=expansion(mult=c(0,0.2)))

low_rna_genes <- weights %>% 
  arrange(value) %>% filter(view != 'Mutations', factor == 'Factor1', view=='mRNA') %>% 
  slice(1:20)
...
high_met_genes <- weights %>% 
  arrange(desc(value)) %>% filter(view != 'Mutations', factor == 'Factor1', view=='Methylation') %>% 
  slice(1:20)

as_tibble(CLL_data$Methylation) %>% mutate(ensgene = rownames(CLL_data$Methylation)) %>%
  # left_join(gene_metadata %>% dplyr::select(ens_id, symbol), by=c('ensgene'='ens_id')) %>%
  filter(ensgene %in% high_met_genes$feature) %>%
  gather(key='sample',value='gexp', -ensgene) %>%
  left_join(CLL_metadata, by='sample') %>% 
  filter(!is.na(IGHV)) %>% mutate(IGHV=factor(IGHV)) %>%
  ggplot(aes(x=IGHV, y=gexp)) +
  geom_boxplot() + facet_wrap(~ensgene, scales='free_y') + stat_compare_means(method='t.test') +
  scale_y_continuous(expand=expansion(mult=c(0,0.2)))

low_met_genes <- weights %>% 
  arrange(value) %>% filter(view != 'Mutations', factor == 'Factor1', view=='Methylation') %>% 
  slice(1:20)

as_tibble(CLL_data$Methylation) %>% mutate(ensgene = rownames(CLL_data$Methylation)) %>%
  # left_join(gene_metadata %>% dplyr::select(ens_id, symbol), by=c('ensgene'='ens_id')) %>%
  filter(ensgene %in% low_met_genes$feature) %>%
  gather(key='sample',value='gexp', -ensgene) %>%
  left_join(CLL_metadata, by='sample') %>% 
  filter(!is.na(IGHV)) %>% mutate(IGHV=factor(IGHV)) %>%
  ggplot(aes(x=IGHV, y=gexp)) +
...
```{r}
high_rna_genes <- weights %>% 
  arrange(desc(value)) %>% filter(view != 'Mutations', factor == 'Factor3', view=='mRNA') %>% 
  slice(1:20)
...
as_tibble(CLL_data$mRNA) %>% mutate(ensgene = rownames(CLL_data$mRNA)) %>%
  ...
  gather(key='sample',value='gexp', -ensgene, -symbol) %>%
  left_join(CLL_metadata, by='sample') %>% 
  filter(!is.na(trisomy12)) %>% mutate(trisomy12=factor(trisomy12)) %>%
  ggplot(aes(x=trisomy12, y=gexp)) +
  geom_boxplot() + facet_wrap(~symbol, scales='free_y') + stat_compare_means(method='t.test') +
  scale_y_log10(expand=expansion(mult=c(0,0.2)))

low_rna_genes <- weights %>% 
  arrange(value) %>% filter(view != 'Mutations', factor == 'Factor3', view=='mRNA') %>% 
  slice(1:20)

as_tibble(CLL_data$mRNA) %>% mutate(ensgene = rownames(CLL_data$mRNA)) %>%
  left_join(gene_metadata %>% dplyr::select(ens_id, symbol), by=c('ensgene'='ens_id')) %>%
  filter(symbol %in% low_rna_genes$feature) %>%
  gather(key='sample',value='gexp', -ensgene, -symbol) %>%
  left_join(CLL_metadata, by='sample') %>% 
  filter(!is.na(trisomy12)) %>% mutate(trisomy12=factor(trisomy12)) %>%
  ggplot(aes(x=trisomy12, y=gexp)) +
